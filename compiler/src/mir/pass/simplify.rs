use {
    crate::{
        mir::{
            BinOp, Body, CastKind, ConstValue, LocalDecls, MirPass, Operand, Rvalue, ScalarRepr,
            Statement, SwitchTargets, Terminator, UnOp,
        },
        Session, Tx,
    },
    std::num::NonZeroU8,
...
impl<'tcx> MirPass<'tcx> for Simplify {
    ...
    fn run_pass(&self, tcx: Tx<'tcx>, body: &mut Body<'tcx>) {
        ...
        for block in &mut body.basic_blocks {
            ...
            simplify_duplicate_switch_targets(block.terminator_mut());
        }
    }
}
